...
static int parse_full_name_option(char* p, char* val)
{
    if (str_equal(("gen"), &p))
    {
        cmdline->gen = 1;
    ...
    {
        cmdline->kbps = atoi(val);
    }
    else if (str_equal(("maxframes"), &p))
    {
        cmdline->max_frames = atoi(val);
    }
    else if (str_equal(("threads"), &p))
    {
        cmdline->threads = atoi(val);
    }
    else if (str_equal(("speed"), &p))
    {
        cmdline->speed = atoi(val);
    }
    ...
    else if (str_equal(("output"), &p))
    {
        strcpy(cmdline->output_file, val);
    }
    else if (str_equal(("input"), &p))
    {
        strcpy(cmdline->input_file, val);
    }
    else if (str_equal(("recon"), &p))
    {
        strcpy(cmdline->recon_file, val);
    }
    else
    {
...
static int read_cmdline_options(int argc, char *argv[])
{
    ...
    for (i = 1; i < argc; i++)
    {
        ...
        if (*p++ == '-')
        {
            switch (*p)
            {
                case '-': parse_full_name_option(p+1, argv[++i]); break;
                case 'o': strcpy(cmdline->output_file, argv[++i]); break;
                case 'i': strcpy(cmdline->input_file, argv[++i]); break;
                case 'r': strcpy(cmdline->input_file, argv[++i]); break;
                default: break;
            }
        } 
        else
    ...
    if (!cmdline->input_file && !cmdline->gen)
    {
        printf("Usage:\n"
               ...
               "Options:\n"
               "    --input,  -i     - Original yuv sequence.\n"
               "    --output, -o     - output bit-stream <output.264>.\n"
...
void init_options()
{
  cmdline->input_file = malloc(sizeof(char)*MAX_FILE_NAME_LENGTH);
  cmdline->output_file = malloc(sizeof(char)*MAX_FILE_NAME_LENGTH);
  cmdline->recon_file = malloc(sizeof(char)*MAX_FILE_NAME_LENGTH);
}

void destory_options()
{
  free(cmdline->input_file);
  free(cmdline->output_file);
  free(cmdline->recon_file);
}
